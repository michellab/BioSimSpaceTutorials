...
def create_restraints(force):
    ...
    restraints = [[BSS.Metadynamics.Restraint(0.64*nm, 0), BSS.Metadynamics.Restraint(1.1*rad, 0), BSS.Metadynamics.Restraint(0.56*nm, 0)], # initial
                  [BSS.Metadynamics.Restraint(0.64*nm, force), BSS.Metadynamics.Restraint(1.1*rad, force), BSS.Metadynamics.Restraint(0.56*nm, force)], # apply force
                  [BSS.Metadynamics.Restraint(0*nm, force), BSS.Metadynamics.Restraint(1.1*rad, force), BSS.Metadynamics.Restraint(0.4*nm, force)], # steering
...
def create_protocol(system, reference, steering_runtime, total_runtime, force):
    ...
    protocol = BSS.Protocol.Steering([rmsd_cv, torsion_cv, distance_cv], schedule, restraints, runtime=total_runtime,
...
def run_sMD(topology, coordinates, reference, steering_runtime, total_runtime, force):
    ...
    protocol = create_protocol(system, reference, steering_runtime, total_runtime, force)

    process = BSS.Process.Amber(system, protocol, exe=f'{os.environ["AMBERHOME"]}/bin/pmemd.cuda')
    process.start()
    process.wait()

    files = ['amber.nc', 'COLVAR', 'amber.rst7', 'plumed.dat', 'reference_1.pdb', 'amber.out']
...
def __main__():
    parser = ArgumentParser(description='Run multi-CV steered MD, steering PTP1B from inactive conformation (open WPD loop) to active conformation (closed WPD loop).')
