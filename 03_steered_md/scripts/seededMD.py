...
def load_system(snapshot, phosphate_parameters, output_dir=""):
    ...
    peptide_parm = BSS.Parameters.ff14SB(
        peptide,
        leap_commands=[f"addPath {phosphate_parameters}", "source leaprc.phosaa10"],
    ).getMolecule()
    ...
    solvated = BSS.Solvent.tip3p(
        new_system, shell=10 * BSS.Units.Length.angstrom, ion_conc=0.15, is_neutral=True
    )
    BSS.IO.saveMolecules(f"{output_dir}system", solvated, ["prm7", "rst7"])
    solvated = BSS.IO.readMolecules(
        [f"{output_dir}system.prm7", f"{output_dir}system.rst7"]
    )
...
def __main__():
    ...
    parser.add_argument(
        "--snapshot", type=str, required=True, help="path to snapshot PDB"
    )
    parser.add_argument(
        "--phosphate_params",
        type=str,
        required=True,
        help="path to folder that contains additional phospho residue parameters",
    )
    parser.add_argument(
        "--output",
        type=str,
