...
CPUQUEUE="serial"
GPUQUEUE="GTX1080,GTX980"
export BSSHOME="/export/users/jscheen/biosimspace.app"
export TMPDIR="./tmp/"
export REMOVE_SIM_DATA="True" # removes trajectory data to save memory. Set to False to review simulations.
...
#echo $tasks
ID1=$(sbatch --parsable --array [0-$tasks] --partition=$GPUQUEUE --exclude node01,node02,node04 --ntasks=1 --gres=gpu:1 --time=$PARAMTIME --job-name=ligprep --output=logs/ligprep_%A_%a.out scripts/BSSligprep.sh)
...
do
    ...
    ID2=$(sbatch --parsable --dependency=afterany:${ID1} --partition=$CPUQUEUE --exclude node01,node02,node04 --ntasks=1 --time=$PREPTIME --job-name=prepFEP --output=logs/prepFEP_%A.out scripts/BSSprepFEP.sh ${ligpair[0]} ${ligpair[1]})
    ...
    tasks=$(( 2*${ligpair[2]} - 1 ))
    ID3=$(sbatch --parsable --dependency=afterany:${ID2} --partition=$GPUQUEUE --exclude node04 --ntasks=1 --gres=gpu:1 --time=$RUNTIME --job-name=runFEP --output=logs/runFEP_%A_%a.out --array=[0-$tasks] scripts/BSSrunFEP.sh ${ligpair[0]} ${ligpair[1]} ${ligpair[3]} ${ligpair[4]})
    ...
    ID4=$(sbatch --parsable --dependency=afterany:${ID3} --partition=$CPUQUEUE --exclude node04 --ntasks=1 --time=$ANALYSISTIME --job-name=analyseFEP --output=logs/analyseFEP_%A.out scripts/BSSanalyseFEP.sh ${ligpair[0]} ${ligpair[1]} ${ligpair[4]})
