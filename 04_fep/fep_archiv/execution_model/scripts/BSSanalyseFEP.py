...
def plotOverlapMatrix(overlap_matrix, savepath):
    ...
    cbar_kws = dict(ticks=[0.025, 0.1, 0.3, 0.8], label="Phase space overlap")
    ax = sns.heatmap(
        ovlp_mtx,
        annot=False,
        fmt=".2f",
...
if data_point in data_entries:
    raise Exception(
        f"Results for this run are already in {results_file_path}. Exiting."
    )
...
if overlap_matrix_bound:
    np.save(
        f"logs/overlap_bound_{sys.argv[1]}~{sys.argv[2]}",
        np.matrix(overlap_matrix_bound),
    )
    plotOverlapMatrix(
        np.matrix(overlap_matrix_bound),
        f"logs/overlap_bound_{sys.argv[1]}~{sys.argv[2]}.png",
    )
else:
    print("Failed to write overlap matrix for bound leg.")
if overlap_matrix_free:
    np.save(
        f"logs/overlap_free_{sys.argv[1]}~{sys.argv[2]}", np.matrix(overlap_matrix_free)
    )
    plotOverlapMatrix(
        np.matrix(overlap_matrix_free),
        f"logs/overlap_free_{sys.argv[1]}~{sys.argv[2]}.png",
    )
else:
