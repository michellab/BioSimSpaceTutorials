...
def test_no_file_passed(executable):
    ...
    error_string = (
        b"You must give at least one networkanalysis networkx compatible file"
    )
...
def test_no_target_compound(executable, graph_file):
    ...
    error_string = (
        b"No target compound given, using the first compound in the node list:"
    )
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, stderr = p.communicate()
    assert error_string in stderr
...
def test_wrong_target_compound(executable, graph_file):
    cmd = [
        ...
        executable,
        graph_file,
        "--target_compound=FXR20",
        "--network_output=tests/io/test_out.dat",
    ]
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, stderr = p.communicate()
...
def test_jupyter_notebook(executable, graph_file):
    try:
        ...
        warnings.warn(
            UserWarning(
                "The Jupyter notebook is not available, so test_jupyter_notebook will be skipped"
            )
        )
    filename = os.path.join(os.getcwd(), "tests", "io", "test_out.dat")
