...
if not os.path.isfile(f"{input_dir}/solvated.prm7"):
    ...
    solvated = BSS.IO.readMolecules(
        [f"{input_dir}/solvated.rst7", f"{input_dir}/solvated.prm7"]
    )
...
funnel_cv = BSS.Metadynamics.CollectiveVariable.Funnel(
    p1, p2, upper_bound=new_upper_bound
)
...
process = BSS.Process.OpenMM(
    equilibrated, protocol, platform="CUDA", work_dir="fun-metaD"
)
...
BSS.IO.saveMolecules("final", finished, ["PDB", "RST7", "PRM7"])
